...
BackgroundColor = Union[Literal["random", "last_sample", "black", "white"], Float[Tensor, "1"], Float[Tensor, "*bs 1"]]
BACKGROUND_COLOR_OVERRIDE: Optional[Float[Tensor, "1"]] = None
...
class BinaryRenderer(nn.Module):#liuyuhan
    ...
    def get_background_color(
        ...
        if BACKGROUND_COLOR_OVERRIDE is not None:
            background_color = BACKGROUND_COLOR_OVERRIDE
        if isinstance(background_color, str) and background_color in colors.COLORS_DICT:
            background_color = torch.tensor([1.0 if background_color == "white" else 0.0])  # 修改: 二进制处理
            #background_color = colors.COLORS_DICT[background_color][Qin]
        assert isinstance(background_color, Tensor)
